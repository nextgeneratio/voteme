...
static int read_max_candidates(void)
{
    ...
    if (!fp) return 500; // default consistent with admin.c
    char line[256]; int max_candidates = 500;
...
static void generate_next_candidate_number(char *out, size_t outsz)
{
    ...
    int next = max_num + 1;
    char tmp[32];
    if (next < 1000) snprintf(tmp, sizeof tmp, "C%03d", next);
    else snprintf(tmp, sizeof tmp, "C%d", next);
...
static bool candidate_nic_exists(const char *nic, const char *except_candidate_number)
{
    ...
    while (fgets(line, sizeof line, f))
    {
        if (strncmp(line, "candidate_number,", 18) == 0) continue;
        char *p = line; char *fields[5] = {0}; int idx = 0;
        for (; idx < 5; ++idx)
        {
            while (*p == ' ') p++;
            fields[idx] = p;
            char *c = strchr(p, ',');
            if (c) { *c = '\0'; p = c + 1; }
            else { char *nl = strchr(p, '\n'); if (nl) *nl = '\0'; idx++; break; }
        }
        for (int i = 0; i < 5 && fields[i]; ++i) trim(fields[i]);
        if (idx >= 5 && fields[4])
        {
            if (except_candidate_number && fields[0] && strcmp(fields[0], except_candidate_number) == 0)
                continue;
            if (strcmp(fields[4], nic) == 0) { fclose(f); return true; }
        }
    }
    fclose(f);
    return false;
...
static void print_available_districts(void)
{
    ...
    if (!fp) { printf("(District list not available)\n"); return; }
    char line[512]; int header = 0;
    while (fgets(line, sizeof line, fp))
    {
        if (!header) { header = 1; if (strncmp(line, "district_id,", 12) == 0) continue; }
        char *p = line; while (*p == ' ' || *p == '\t') p++;
        char *comma = strchr(p, ','); if (!comma) continue; *comma = '\0';
        char *id = p; char *name = comma + 1; char *nl = strchr(name, '\n'); if (nl) *nl = '\0';
        trim(id); trim(name);
...
{
    FILE *fp = fopen(DISTRICT_FILE, "r");
    if (!fp) return true; // don't block if list missing
    char line[512]; int header = 0;
    while (fgets(line, sizeof line, fp))
    {
        if (!header) { header = 1; if (strncmp(line, "district_id,", 12) == 0) continue; }
        char *p = line; while (*p == ' ' || *p == '\t') p++;
        char *comma = strchr(p, ','); if (!comma) continue; *comma = '\0'; trim(p);
        if (strcmp(p, district_id) == 0) { fclose(fp); return true; }
    }
...
static void prompt_for_district_id(char *out, size_t outsz)
{
    ...
    while (1)
    {
        ...
        if (out[0] == '\0') { printf("District ID cannot be empty.\n"); continue; }
        if (!is_valid_district_id(out)) { printf("Invalid District ID. Choose from the list above.\n"); continue; }
        break;
    }
}
...
static void print_available_parties(void)
{
    ...
    if (!fp) { printf("(Party list not available)\n"); return; }
    char line[512]; int header = 0;
    while (fgets(line, sizeof line, fp))
    {
        if (!header) { header = 1; if (strncmp(line, "party_id,", 9) == 0) continue; }
        char *p = line; while (*p == ' ' || *p == '\t') p++;
        char *comma = strchr(p, ','); if (!comma) continue; *comma = '\0';
        char *id = p; char *name = comma + 1; char *nl = strchr(name, '\n'); if (nl) *nl = '\0';
        trim(id); trim(name);
...
static bool is_valid_party_id(const char *party_id)
{
    ...
    while (fgets(line, sizeof line, fp))
    {
        ...
        char *p = line; while (*p == ' ' || *p == '\t') p++;
        char *comma = strchr(p, ','); if (!comma) continue; *comma = '\0'; trim(p);
        if (strcmp(p, party_id) == 0) { fclose(fp); return true; }
    }
    fclose(fp); return false;
}
...
static void prompt_for_party_id(char *out, size_t outsz)
{
    ...
    while (1)
    {
        ...
        if (out[0] == '\0') { printf("Party ID cannot be empty.\n"); continue; }
        if (!is_valid_party_id(out)) { printf("Invalid Party ID. Choose from the list above.\n"); continue; }
        break;
    }
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        case 2: readCandidates(); break;
        case 3: updateCandidate(); break;
...
static void createCandidate(void)
{
    ...
    if (!nic[0]) { printf("NIC cannot be empty.\n"); return; }
    if (candidate_nic_exists(nic, NULL)) { printf("Error: NIC already exists.\n"); return; }

    int rc = create_candidate(candidate_number, name, party_id, district_id, nic);
    if (rc == DATA_SUCCESS) printf("✅ Candidate Registered Successfully.\n");
    else printf("❌ Failed to register candidate. %s\n", get_last_error());
}
...
{
    FILE *f = fopen(APPROVED_CANDIDATES_FILE, "r");
    if (!f) { printf("No candidate data found.\n"); return; }
    printf("\n======= APPROVED CANDIDATES =======\n");
...
static void updateCandidate(void)
{
    ...
    if (!candidate_number[0]) { printf("Candidate Number is required.\n"); return; }

    char *current = read_candidate(candidate_number);
    if (!current) { printf("Candidate not found.\n"); return; }
    printf("Current record: %s\n", current);
    free(current);

    printf("Choose field to update:\n");
    printf("1. Name\n2. Party ID\n3. District ID\n4. NIC\nEnter choice (1-4): ");
    int choice = 0; char buf[16];
    if (!fgets(buf, sizeof buf, stdin)) return; choice = atoi(buf);
    ...
    if (!new_value[0]) { printf("Value cannot be empty.\n"); return; }
    ...
    if (rc == DATA_SUCCESS) printf("✅ Candidate Updated Successfully.\n");
    else printf("❌ Failed to update candidate. %s\n", get_last_error());
}
...
static void deleteCandidate(void)
{
    ...
    if (rc == DATA_SUCCESS) printf("✅ Candidate Deleted Successfully.\n");
    else printf("❌ Failed to delete candidate or not found. %s\n", get_last_error());
}
