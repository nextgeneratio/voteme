...
static int validate_string_input(const char *str, const char *param_name, int max_length) {
    if (!str) {
    ...
    if (strlen(str) >= max_length) {
        set_error_message("Error: %s exceeds maximum length of %d characters", param_name, max_length);
        return 0;
    }
