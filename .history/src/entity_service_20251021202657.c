...
int create_candidate_struct(const Candidate *c)
{
    if (!c)
    {
        set_error_message("Error: NULL candidate");
...
{
    if (!candidate_number || !out)
    {
        set_error_message("Error: invalid args");
        return DATA_ERROR_INVALID_INPUT;
    }
    ...
    if (!ok)
    {
        set_error_message("Error: parse candidate failed");
...
int create_voter_struct(const Voter *v)
{
    if (!v)
    {
        set_error_message("Error: NULL voter");
        return DATA_ERROR_INVALID_INPUT;
    }
    char buf[256];
...
int read_voter_struct(const char *voting_number, Voter *out)
{
    ...
    if (!ok)
    {
        set_error_message("Error: parse voter failed");
        return DATA_ERROR_MALFORMED_DATA;
    }
    return DATA_SUCCESS;
}
