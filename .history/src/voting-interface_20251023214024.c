...
int vote_for_candidate_interactive(void)
{
    ...
    char *party_rec = read_party(party_id);
    if (!party_rec)
    {
        int ccount = count_candidates_for_party(candidates_path, party_id);
        if (ccount <= 0)
        {
            printf("Party ID '%s' not found and no candidates registered for it.\n", party_id);
            return DATA_ERROR_RECORD_NOT_FOUND;
        }
        printf("Note: Party ID '%s' not in party list, proceeding based on candidate data.\n", party_id);
    }
    else
    {
        free(party_rec);
        // Optional presence check in party list
        (void)list_parties_and_exists(parties_path, party_id);
    }
