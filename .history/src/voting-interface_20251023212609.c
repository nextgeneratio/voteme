...
static bool list_parties_and_exists(const char *path, const char *party_id) {
    ...
    FILE *f = fopen(path, "r");
    if (!f) {
        fprintf(stderr, "Error: cannot open %s (code %d)\n", path, get_last_error());
        return false;
    }
    ...
    while (read_csv_line(f, line, sizeof(line))) {
        ...
        char fields[8][MAX_FIELD_LENGTH];
        int nf = split_csv_fields(line, fields, 8);
        if (nf >= 2) {
...
static int list_candidates_for_party(const char *candidates_path, const char *party_id, char out_ids[][MAX_FIELD_LENGTH], int max_ids) {
    ...
    while (read_csv_line(f, line, sizeof(line))) {
        ...
        if (nf >= 3 && strcmp(fields[2], party_id) == 0) {
            printf("  %s - %s\n", fields[0], fields[1]);
            if (count < max_ids) {
                strncpy(out_ids[count], fields[0], MAX_FIELD_LENGTH - 1);
                out_ids[count][MAX_FIELD_LENGTH - 1] = '\0';
                count++;
            }
        }
    }
...
static bool id_in_list(const char *id, char ids[][MAX_FIELD_LENGTH], int n) {
    for (int i = 0; i < n; ++i) {
...
int vote_for_candidate_interactive(void) {
    ...
    if (!list_parties_and_exists(parties_path, NULL)) {
        return get_last_error() ? get_last_error() : DATA_ERR_IO;
    }
    ...
    if (!read_party(parties_path, buf)) {
