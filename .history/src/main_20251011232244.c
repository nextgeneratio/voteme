...
void handle_voter_management(void) {
    ...
    do {
        ...
        case 1: {
            ...
            char voting_number[20], nic[20], name[100], district_id[20];
            
            printf("Enter Voting Number: ");
            if (scanf("%19s", voting_number) != 1) {
            ...
            if (create_voter(voting_number, name, nic, district_id) == DATA_SUCCESS) {
                printf(BRIGHT_GREEN "✅ Voter added successfully!\n" RESET);
            } else {
                printf(RED "❌ Failed to add voter!\n" RESET);
            }
            pause_for_user();
            break;
        }
