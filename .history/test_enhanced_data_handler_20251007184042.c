...
void test_enhanced_crud_operations() {
    ...
    ASSERT(create_record("test_data/crud_test.csv", "header1,header2") == DATA_SUCCESS, 
...
void test_entity_specific_functions() {
    ...
    // Setup entity test files
    FILE *fp = fopen("test_data/approved_candidates.txt", "w");
    if (fp) {
        fprintf(fp, "candidate_number, name, party_id, district_id, nic\n");
        fclose(fp);
    }
    
    fp = fopen("test_data/approved_voters.txt", "w");
    if (fp) {
        fprintf(fp, "voting_number, name, nic, district_id\n");
        fclose(fp);
    }
    
    fp = fopen("test_data/voter_count.txt", "w");
    if (fp) {
        fprintf(fp, "voting_number, candidate_number, party_id, district_id, count\n");
        fclose(fp);
    }
    
    // Change directory to src for relative paths to work
    char original_dir[1024];
    getcwd(original_dir, sizeof(original_dir));
    chdir("test_data");
    system("mkdir -p ../../data");
    system("cp approved_candidates.txt ../../data/");
    system("cp approved_voters.txt ../../data/");
    system("cp voter_count.txt ../../data/");
    chdir(original_dir);
