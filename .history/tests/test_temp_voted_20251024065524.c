...
static void parse_triplet(const char *rec, char *out_v, char *out_c, char *out_p)
{
    ...
    if (!a || !b) { out_v[0]=out_c[0]=out_p[0]='\0'; return; }
    size_t lv = (size_t)(a - rec);
    size_t lc = (size_t)(b - a - 1);
    strncpy(out_v, rec, lv); out_v[lv] = '\0'; trim(out_v);
    strncpy(out_c, a+1, lc); out_c[lc] = '\0'; trim(out_c);
    strncpy(out_p, b+1, MAX_LINE_LENGTH-1); out_p[MAX_LINE_LENGTH-1]='\0'; trim(out_p);
}
