...
// Removed unused count_voter_rows to avoid compiler warnings
...
static bool nic_exists(const char *nic)
{
    ...
    {
        if (first && strncmp(line, "voting_number,", 14) == 0)
        {
            first = false;
            continue;
        }
...
static void generate_next_voting_number(char out[16])
{
    int maxn = 0;
    FILE *fp = fopen(VOTERS_FILE, "r");
    if (fp)
...
int main(void)
{
    ...
    // Pre-compute and show the assigned ID so the user knows it upfront
    char id[16];
    generate_next_voting_number(id);
    printf("Assigned Voting ID: %s\n\n", id);
    ...
    list_districts();
    printf("\nEnter district id: ");
    if (!fgets(did, sizeof(did), stdin))
        return 1;
    trim_newline(did);
    if (!valid_district_id(did))
    {
        printf("\n[!] Invalid district id.\n");
        press_enter_to_exit();
        return 1;
