...
int main(void) {
    ...
    if ((fp = fopen("data/approved_voters.txt", "r")) == NULL) {
        ...
        if (!fp) { perror("approved_voters.txt"); return 1; }
    ...
    if ((fp = fopen("data/approved_candidates.txt", "r")) == NULL) {
        ...
        if (!fp) { perror("approved_candidates.txt"); return 1; }
    ...
    if (rc != DATA_SUCCESS) { printf("FAIL: create_voter rc=%d err=%s\n", rc, get_last_error()); return 1; }

    rc = create_candidate("C100","Test Candidate","P100","D01","NIC200");
    if (rc != DATA_SUCCESS) { printf("FAIL: create_candidate rc=%d err=%s\n", rc, get_last_error()); return 1; }

    char* v = read_voter("V100");
    if (!v) { printf("FAIL: read_voter err=%s\n", get_last_error()); return 1; }
    free(v);

    char* c = read_candidate("C100");
    if (!c) { printf("FAIL: read_candidate err=%s\n", get_last_error()); return 1; }
    free(c);
